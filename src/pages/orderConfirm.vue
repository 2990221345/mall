        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked': checkedItem == index}" @click="checkedItem = index" v-for="(item, index) in list" :key="index">
                <h2>{{ item.receiverName }}</h2>
                <div class="phone">{{ item.receiverMobile }}</div>
                <div class="street">
                  {{ item.receiverProvince+' '+item.receiverCity +' '+item.receiverDistrict+' '}}<br>
                  {{item.receiverAddress}}
                  <br />
                </div>
                <div class="action">
                  <a href="javascript:;" class="fl" @click="delAdderss(item)">
    ...
    >
      <template v-slot:body>
        <div class="edit-wrap">
          ...
          <div class="item">
            <select name="probince" v-model="checkItem.receiverProvince">
            ...
            <select name="probince" v-model="checkItem.receiverDistrict">
  ...
  methods: {
    ...
    submitAddress() {
      ...
      if (userAction == 0 || userAction == 1) {
        ...
        if(!receiverName){
          Msgerr = '请输入收货人名称'
        }else if(!receiverMobile || !/^1(3[0-9]|4[01456879]|5[0-35-9]|6[2567]|7[0-8]|8[0-9]|9[0-35-9])\d{8}$/.test(receiverMobile)){
          Msgerr = '请输入正确格式的手机号'
        ...
        params = {
          receiverName: this.checkItem.receiverName,
          receiverMobile: this.checkItem.receiverMobile,
          receiverProvince: this.checkItem.receiverProvince,
          receiverCity: this.checkItem.receiverCity,
          receiverDistrict: this.checkItem.receiverDistrict,
          receiverAddress: this.checkItem.receiverAddress,
          receiverZip: this.checkItem.receiverZip
        }
      }
         this.axios[method](url, params)
        .then(() => {
          this.closeModal()
          this.getAddressList() //更新地址
    ...
    getCartList() {
      this.axios.get('/carts').then(res => {
        let list = res.cartProductVoList //商品数组
        this.cartTotalPrice = res.cartTotalPrice //商品总金额
        this.cartList = list.filter(item => item.productSelected) //选中的商品数组
    ...
    // 订单提交
    orderSubmit() {
      let item = this.list[this.checkedItem]
      if(!item){
